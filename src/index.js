...
/*function search(city) {
  ...
  let time = localStorage.getItem("lastSearchTime");

  // Check if there was a previous search within 2 hours
  if (time && now - time < 7200000) { // 7200000 ms = 2 hours
    let weatherData = JSON.parse(localStorage.getItem("weatherData"));
    showtemperature(weatherData);
  } else {
...
function showtemperature(response) {
  if (response.hasOwnProperty("data") && response.data.hasOwnProperty("name")) {
    document.querySelector("#current-city-country").innerHTML = response.data.name;
